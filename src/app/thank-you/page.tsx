...
export default async function Page({ searchParams }: PageProps) {
  ...
  const orderPrice = products.reduce((total, p) => {
    ...
    <main className="relative lg:min-h-full">
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8 lg:py-32 xl:gap-x-24">
        <div className="lg:col-start-2">
          ...
          <h1 className="mt-2 text-4xl font-bold tracking-tight text-gray-900 sm:text-5xç">
            Thanks for ordering
          </h1>
          {order._isPaid ? (
          ...
          <div className="mt-16 text-sm font-medium">
            ...
            <ul className="mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground">
              {(order.products as Product[]).map((p) => {
                const label = PRODUCT_CATEGORIES.find(
                  (c) => c.value === p.category,
                )?.label

                const downloadUrl = (p.product_files as ProductFile)
                  .url as string

                const { image } = p.product_images[0]

                return (
                  <li key={p.id} className="flex space-x-6 py-6">
                    ...
                    <div className="flex-auto flex flex-col justify-between">
                      <div className="space-y-1">
                        <h3 className="text-gray-900">{p.name}</h3>
                      ...
                      {order._isPaid ? (
                        <a
                          href={downloadUrl}
                          download={p.name}
                          className="text-blue-600 hover:underline underline-offset-2 gap-2"
                        >
                          Download asset
                        </a>
                      ) : null}
            ...
            <div className="space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-muted-foreground">
              <div className="flex justify-between">
                <p>Subtotal</p>
                <p className="text-gray-900">{formatPrice(orderPrice)}</p>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-6 text-gray-900">
                ...
                <p className="text-base">{formatPrice(orderPrice + fee)}</p>
              </div>
            </div>
